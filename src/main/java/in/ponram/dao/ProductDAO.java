...
public class ProductDAO {
	...
	public List<Product> findAll() {
		...
		try {
			...
			// Step 2: Query
			String sql = "SELECT product_id,brand_name,product_name,product_category,arrival_date,available_quantity,rate  FROM stock";
			pst = connection.prepareStatement(sql);
	...
	/**
	 * This method is used to get product using brand name from database
	 * @param brand
	 * @return
	 */
	public List<Product> findByBrandName(String brand) {
		...
		try {
			...
			// Step 2: Query
			String sql = "select product_id,brand_name,product_name,product_category,arrival_date,available_quantity,rate from stock where brand_name = ?";
			pst = connection.prepareStatement(sql);
			// Step 3: execute query
